...
void leaf_to_page(leaf_page* src,page_t* dest)
{
	...
	leaf->Reserved = src->Reserved;
	leaf->Page_LSN = src->Reserved;
	leaf->Number_of_Keys = src->Number_of_Keys;
	for(int i =0;i<11;i++) leaf->dummy[i] = src->dummy[i];
...
void page_to_leaf(page_t* src,leaf_page* dest)
{
	...
	dest->Reserved = leaf->Reserved;
	dest->Page_LSN = leaf->Page_LSN;
	for(int i =0;i<11;i++) dest->dummy[i] = leaf->dummy[i];
...
void internal_to_page(internal_page* src,page_t* dest)
{
	...
	internal->Reserved = src->Reserved;
	internal->Page_LSN = src->Page_LSN;
	for(int i =0;i<11;i++) internal->dummy[i] = src->dummy[i];
...
void page_to_internal(page_t* src, internal_page* dest)
{
	...
	dest->Reserved = internal->Reserved;
	dest->Page_LSN = internal->Page_LSN;
	for(int i =0;i<11;i++) dest->dummy[i] = internal->dummy[i];
