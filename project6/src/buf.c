...
int init_db(int size, int flag, int log_num,char* log_path,char* logmsg_path) // LibraryAPI_create buffer list
{
	//int test_id = 0;
	...
	for(int i =0;i<size;i++)
	{
		...
		temp->is_dirty = 0;
		//temp->pinned = 0;
		pthread_mutex_init(&temp->page_mutex,NULL);
		temp->prev = NULL;
		temp->next = NULL;
		// make new buffer
...
void Get_Buffer(int t_id, pagenum_t p_number, page_t* dest, int lock_page)//Get page in buffer_list(buffer version of file_read_page function)
{
	...
	while(tmp->next!=NULL) // find buffer that t_id,p_number(head_to_tail)
	{
		...
		{
			pthread_mutex_lock(&tmp->page_mutex);
	...
	if(can_t_find == 0)//using buffer go to Head
	{
		...
		if(lock_page==0)
		{
			pthread_mutex_unlock(&tmp->page_mutex);
		}
	}
	...
	if(can_t_find == 1) // can't find buffer that t_id,p_number
	{
		...
		while(replace->prev!=NULL)
		{
			...
			{
				pthread_mutex_lock(&replace->page_mutex);
	...
	if(can_t_find==2) // find old buffer to change and go to Head
	{
		...
		if(lock_page==0)
		{
			pthread_mutex_unlock(&replace->page_mutex);
		}
	}
...
void Set_Buffer(int t_id, pagenum_t p_number, page_t* src, int lock_page) //Set page in buffer_list(buffer version of file_write_page function)
{
	pthread_mutex_lock(&buf_mutex);
...
void release_page(int t_id, pagenum_t p_num)
{
	buffer_node* tmp = buf_head;
	while(tmp->next!=NULL)
	{
		if(tmp->table_id==t_id && tmp->page_number == p_num)
		{
			pthread_mutex_unlock(&tmp->page_mutex);
			break;
		}
