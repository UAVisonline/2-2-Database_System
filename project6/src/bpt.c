...
void usage_prototype()
{
	...
	printf("m : <int> <string> -- Modify the value of the page\n");
...
void print_leaves(int table_id)
{
	...
	Get_Buffer(table_id,0,head_page,0);
	page_to_header(head_page,header);
	...
	{
		Get_Buffer(table_id,root_number,page,0);
...
int db_find(int table_id,int64_t key,char* ret_values, int trx_id) // LibraryAPI_find
{
	uint64_t root_number = 0;
	internal_page* internal = (internal_page*)malloc(Page_size);
	...
	{
		Get_Buffer(table_id,root_number,find_page,0);
		...
		Get_Buffer(table_id,root_number,find_page,1);
		...
		for(int i =0;i<find_leaf->Number_of_Keys;i++)
		{
			if(key==find_leaf->records[i].key) // find key
			{
				...
				int ret = lock_acquire(table_id,key,trx_id,0,tmp_obj);
				//if(tmp_obj==NULL) printf("NULLLLL find\n");
				if(ret==0)
				{
				...
				{
					release_page(table_id,root_number);
					lock_wait(tmp_obj);

					release_xact_node(tmp_obj);
					//Get_Buffer(table_id,root_number,find_page,1);
					Get_Buffer(table_id,0,head_page,0);
					page_to_header(head_page,header);
					...
					if(root_number!=0)
					{
						...
						while(TRUE) // go to leaf page that can have a key in page
						{
							...
							{
								root_number = internal->one_more_page_number;
								for(int j =0;j<internal->Number_of_Keys;j++)
								{
									if(key>=internal->pointers[j].key)
									{
										root_number = internal->pointers[j].page_num;
									}
									else
									{
										break;
					...
					Get_Buffer(table_id,root_number,find_page,1);
					page_to_leaf(find_page,find_leaf);
					for(int j =0;j<find_leaf->Number_of_Keys;j++)
					{
						if(key==find_leaf->records[j].key)
						{
							strcpy(ret_values,find_leaf->records[j].value);
							break;
						}
					}
				}
...
int db_update(int table_id, int64_t key, char* values, int trx_id)
{
	...
	else
	{
		...
		Get_Buffer(table_id,root_number,page,1);

		page_to_leaf(page,update_leaf);
		for(int i =0; i<update_leaf->Number_of_Keys;i++)
		{
			if(key==update_leaf->records[i].key)
			{
				lock_t* tmp_obj = (lock_t*)malloc(sizeof(lock_t));
				int ret = lock_acquire(table_id,key,trx_id,1,tmp_obj);
				//if(tmp_obj==NULL) printf("NULLLLL update\n");
				if(ret==0)
				{
					recovery_value_save(tmp_obj,update_leaf->records[i].value);
					strcpy(update_leaf->records[i].value,values);
					leaf_to_page(update_leaf,page);
					release_page(table_id,root_number);
					Set_Buffer(table_id,root_number,page,1);
				}
				else if(ret==1)
				{
					release_page(table_id,root_number);
					...
					Get_Buffer(table_id,root_number,page,1);
					page_to_leaf(page,update_leaf);
					for(int j =0;j<update_leaf->Number_of_Keys;j++)
					{
						if(key==update_leaf->records[j].key)
						{

							recovery_value_save(tmp_obj,update_leaf->records[j].value);
							strcpy(update_leaf->records[j].value,values);
							leaf_to_page(update_leaf,page);
							release_page(table_id,root_number);
							Set_Buffer(table_id,root_number,page,1);
							break;
			...
			else if(key<update_leaf->records[i].key)
			{
				free(header);
				free(internal);
				free(update_leaf);
			...
			else if(key!=update_leaf->records[i].key && i == update_leaf->Number_of_Keys-1)
			{
				free(header);
				free(internal);
				free(update_leaf);
...
int Start_new_tree(int table_id,record* rec)
{
	...
	Get_Buffer(table_id,root_number,tmp,0);
	...
	Set_Buffer(table_id,root_number,tmp,0);
	free(leaf);
	free(tmp);
	...
	Set_Buffer(table_id,0,head_page,0);
	free(head_page);
	free(header);
...
int insert_record_into_leaf(int table_id,pagenum_t number,record* rec)
{
	...
	page_t* page = (page_t*)malloc(Page_size);
	leaf_page* leaf = (leaf_page*)malloc(Page_size);
	Get_Buffer(table_id,number,page,0);
	page_to_leaf(page,leaf);
	record_key = rec->key;
	...
	Set_Buffer(table_id,number,page,0);
	free(leaf);
	free(page);
...
int insert_record_into_leaf_split(int table_id,pagenum_t old_number,record* rec)
{
	...
	Get_Buffer(table_id,new_number,new_page,0);
	
	Get_Buffer(table_id,old_number,old_page,0);
	...
	Set_Buffer(table_id,old_number,old_page,0);
	Set_Buffer(table_id,new_number,new_page,0);
...
int insert_into_parent(int table_id,pagenum_t parent_number,pagenum_t old_number, int64_t new_key,pagenum_t new_number,int leaf_check)
{
	...
	if(parent_number == 0) //page of child has no parent case, we make new root_internal_page
	{
		...
		Get_Buffer(table_id,parent_number,page,0);
		...
		Set_Buffer(table_id,parent_number,page,0);
		...
		if(leaf_check==1) //child_page is leaf
		{
			...
			leaf = (leaf_page*)malloc(Page_size);
			Get_Buffer(table_id,old_number,page,0);
			//file_read_page(old_number,page);
			page_to_leaf(page,leaf);
			leaf->parent_page_num = parent_number;
			leaf_to_page(leaf,page);
			Set_Buffer(table_id,old_number,page,0);
			...
			page = (page_t*)malloc(Page_size);
			leaf = (leaf_page*)malloc(Page_size);
			Get_Buffer(table_id,new_number,page,0);
			//file_read_page(new_number,page);
			page_to_leaf(page,leaf);
			leaf->parent_page_num = parent_number;
			leaf_to_page(leaf,page);
			Set_Buffer(table_id,new_number,page,0);
...
int insert_key_into_internal_split(int table_id, pagenum_t old_number,int64_t new_key,pagenum_t new_number,int left_index)
{
	...
	Get_Buffer(table_id,old_number,old_page,0);
	Get_Buffer(table_id,new_internal_number,new_page,0);
	...
	Get_Buffer(table_id,new_internal->one_more_page_number,child_page,0);
	...
	if(child_internal->Is_Leaf==0)
	{
		...
		Set_Buffer(table_id,new_internal->one_more_page_number,child_page,0);
		//file_write_page(new_internal->one_more_page_number,child_page);
		for(i=0;i<new_internal->Number_of_Keys;i++)
		{
			Get_Buffer(table_id,new_internal->pointers[i].page_num,child_page,0);
			//file_read_page(new_internal->pointers[i].page_num,child_page);
			page_to_internal(child_page,child_internal);
			child_internal->parent_page_num = new_internal_number;
			internal_to_page(child_internal,child_page);
			Set_Buffer(table_id,new_internal->pointers[i].page_num,child_page,0);
	...
	Set_Buffer(table_id,old_number,old_page,0);
	Set_Buffer(table_id,new_internal_number,new_page,0);
	//file_write_page(old_number,old_page);
...
int db_insert(int table_id,int64_t key,char* values) //LibraryAPI_insert
{
	...
	else
	{
		...
		while(TRUE)
		{
			page = (page_t*)malloc(Page_size);
			internal = (internal_page*)malloc(Page_size);
			Get_Buffer(table_id,page_number,page,0);
...
int delete_in_leaf(int table_id,int64_t key,pagenum_t leaf_number)
{
	...
	Get_Buffer(table_id,leaf_number,page,0);
	...
	leaf_to_page(leaf,page);
	Set_Buffer(table_id,leaf_number,page,0);
...
int delayed_merge_leaf(int table_id,pagenum_t parent_number,int64_t prime_key)//leaf(child)->internal(parent) case
{
	...
	Get_Buffer(table_id,parent_number,parent_page,0);
	...
	Get_Buffer(table_id,deleted_number,child_page,0);
	...
	if(deleted_number == parent_internal->one_more_page_number)
	{
		...
		Get_Buffer(table_id,real_delete_number,copy_page,0);
		...
		leaf_to_page(child_leaf,child_page);
		Set_Buffer(table_id,deleted_number,child_page,0);
	...
	else
	{
		...
		Get_Buffer(table_id,change_leaf_number,changed_page,0);
		...
		Set_Buffer(table_id,change_leaf_number,changed_page,0);
	...
	Set_Buffer(table_id,parent_number,parent_page,0);
	...
	else if(parent_internal->Number_of_Keys==0) // we delete parent_page
	{
		if(grand_parent_number==0) // parent_page is root
		{
			...
			Get_Buffer(table_id,parent_internal->one_more_page_number,page,0);	
			//file_read_page(parent_internal->one_more_page_number,page);
			page_to_leaf(page,leaf);
			leaf->parent_page_num = 0;
			leaf_to_page(leaf,page);
			Set_Buffer(table_id,parent_internal->one_more_page_number,page,0);
		...
		else
		{
			...
			Get_Buffer(table_id,grand_parent_number,grand_page,0);
			...
			free(grand_internal);
			free(grand_page);
			Get_Buffer(table_id,uncle_number,uncle_page,0);
...
int delayed_merge_internal(int table_id,pagenum_t parent_number, int64_t prime_key,pagenum_t prime_pagenum)//internal(child)->internal(parent)
{
	...
	if(deleted_number == parent_internal->one_more_page_number) //left_most page case
	{
		...
		Get_Buffer(table_id,child_internal->pointers[0].page_num,tmp_page,0);
		...
		if(tmp_internal->Is_Leaf==0)
		{
			for(int i =0;i<child_internal->Number_of_Keys;i++)
			{
				...
				Get_Buffer(table_id,child_internal->pointers[i].page_num,tmp_page,0);
				page_to_internal(tmp_page,tmp_internal);
				tmp_internal->parent_page_num = deleted_number;
				internal_to_page(tmp_internal,tmp_page);
				Set_Buffer(table_id,child_internal->pointers[i].page_num,tmp_page,0);
	...
	else
	{
		...
		Get_Buffer(table_id,change_internal_number,changed_page,0);
...
int delayed_redistribute_internal(int table_id,pagenum_t parent_number,int64_t prime_key,pagenum_t uncle_number)
{
	...
	Get_Buffer(table_id,solo_number,solo_page,0);
	...
	Set_Buffer(table_id,parent_number,parent_page,0);
	Set_Buffer(table_id,solo_number,solo_page,0);
	Set_Buffer(table_id,uncle_number,uncle_page,0);
	//file_write_page(parent_number,parent_page);
	...
	if(tmp_number!=0)
	{
		...
		Get_Buffer(table_id,tmp_number,tmp_page,0);
		page_to_internal(tmp_page,tmp_internal);
		if(tmp_internal->Is_Leaf==0)
		{
			tmp_internal->parent_page_num = solo_number;
			internal_to_page(tmp_internal,tmp_page);
			Set_Buffer(table_id,tmp_number,tmp_page,0);
...
int db_delete(int table_id,int64_t key)//LibraryAPI_delete
{
	...
	else
	{
		...
		Get_Buffer(table_id,page_num,page,0);
		page_to_internal(page,internal);
		while(TRUE)
		{
			if(internal->Is_Leaf==1)
