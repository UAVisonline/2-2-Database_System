...
pagenum_t file_alloc_page()
{
	...
	Get_Buffer(current_table_id,0,head_page,0); //Read Header_page(table_id)
	...
	if(ret_number!=0)
	{
		...
		Get_Buffer(current_table_id,ret_number,page,0); //Read page(table_id) in Buffer
	...
	Set_Buffer(current_table_id,0,head_page,0); // Write page(table_id) in Buffer
	free(head_page);
	free(header);
...
void file_free_page(pagenum_t pagenum)
{
	...
	page_t* head_page = (page_t*)malloc(Page_size);
	header_page* header = (header_page*)malloc(Page_size);
	Get_Buffer(current_table_id,0,head_page,0); //Read header(table_id) in Buffer
	page_to_header(head_page,header);
	...
	Set_Buffer(current_table_id,pagenum,free_page,0);//Write page_deleted(table_id) in Buffer(empty page insert)
	free(free_page); 
	...
	if(header->next_free_page==0)
	{
		...
		header_to_page(header,head_page);
		Set_Buffer(current_table_id,0,head_page,0);//Write header_page(table_id) in Buffer
	}	
	else
	{
		previous_page_num = header->next_free_page;
		page_t* dummy = (page_t*)malloc(Page_size);
		Get_Buffer(current_table_id,previous_page_num,dummy,0); // Read free_page(table_id) in Buffer
		...
		Set_Buffer(current_table_id,previous_page_num,dummy,0); // Write free_page(table_id) that changed information in Buffer
		free(dummy);
	}
	free(head_page);
	free(header);
...
int open_table(char* file_pathname)//LibraryAPI_open_table
{
	...
	path_to_int = (char*)malloc(sizeof(file_pathname)-4);
	for(int i =4;i<sizeof(file_pathname);i++)
	{
		path_to_int[i-4] = file_pathname[i];
	}	
	p_id = atoi(path_to_int);
	free(path_to_int);
	if(p_id==0)
	{
		return -1;
	...
	while(1)
	{
		if(curr->table_name==NULL) //No table that named file_pathname 
		{
			...
			curr->table_id = p_id; // set table_id
