...
int trx_begin() //Library_for_transaction
{
	...
	pthread_mutex_init(&temp->xact_mutex,NULL);
	if(trx_head == NULL)
	{
		temp->prev = NULL;
		temp->next = NULL;
			...
			while(lock_temp!=NULL)
			{
				...
				if(delete_lock->recovery_value != NULL)
				{
					...
					for(int i =0;i<update_leaf->Number_of_Keys;i++)
					{
						...
						{
							//printf("%s\n%s\n",update_leaf->records[i].value,delete_lock->recovery_value);
							strcpy(update_leaf->records[i].value,delete_lock->recovery_value);
							leaf_to_page(update_leaf,page);
							Set_Buffer(table_id,root_number,page,0);
							break;
						}
					}
					free(update_leaf);
...
int lock_acquire(int _table_id,int64_t _key,int _trx_id, int _lock_mode,lock_t* lock_temp)
{
	pthread_mutex_lock(&trx_mutex);
	int wait_status = 0;
	...
	while(xact_temp!=NULL)
	{
		...
		{
			pthread_mutex_lock(&xact_temp->xact_mutex);
			break;
		}
	...
	lock_temp->prev = NULL;
	lock_temp->next = NULL;
	lock_temp->entry = entry_reference;
	lock_temp->same_thread_next = NULL;
	lock_temp->hash = hash_value;
	lock_temp->owner_trx_id = _trx_id;
	lock_temp->lock_mode = _lock_mode;
	lock_temp->can_have_recovery = 1;
	lock_temp->wait = 0;
	lock_temp->deadlock_check = 1;
	lock_temp->recovery_value = NULL;
	if(obj_reference==NULL)
	{
		obj_reference = lock_temp;
	...
	else
	{
		...
		obj_reference->next = lock_temp;
		lock_temp->prev = obj_reference;
	}
	
	if(tmp_lock_obj==NULL)
	{
		xact_temp->head_lock_object = xact_temp->tail_lock_object = lock_temp;
	}
	else
	{
		tmp_lock_obj->same_thread_next = lock_temp;
		xact_temp->tail_lock_object = lock_temp;
	}

	if(lock_temp->lock_mode==0)
	{
		lock_t* tmp = lock_temp->prev;
		while(tmp!=NULL)
		{
			if(tmp->owner_trx_id!=lock_temp->owner_trx_id)
			{
				if(tmp->lock_mode==0)
				{
					tmp = tmp->prev;
			...
			else if(tmp->owner_trx_id==lock_temp->owner_trx_id)
			{
				tmp = tmp->prev;
			}
		}
	}
	else if(lock_temp->lock_mode==1)
	{
		lock_t* tmp = lock_temp->prev;
		while(tmp!=NULL)
		{
	...
	if(wait_status==1)
	{
		if(lock_temp->deadlock_check==1)
		{
			lock_t* prev_obj = lock_temp->prev;
			while(prev_obj!=NULL)
			{
				if(prev_obj->owner_trx_id!=lock_temp->owner_trx_id)
				{
					if(deadlock_detection(prev_obj->owner_trx_id,lock_temp->owner_trx_id)!=0)
					{
	...
	pthread_mutex_unlock(&xact_temp->xact_mutex);
	return 0;
}
...
{
	int hash = lock_obj->hash;
	int wait_status = 0;
	pthread_cond_wait(&hash_cond[hash],&hash_mutex);
...
void release_xact_node(lock_t* lock_obj)
{
	int obj_trx_id = lock_obj->owner_trx_id;
	xact_node* xact_temp = trx_head;
	while(xact_temp!=NULL)
	{
		if(xact_temp->trx_id==obj_trx_id)
		{
			pthread_mutex_unlock(&xact_temp->xact_mutex);
			break;
		}
